...
<template>
  <div :class="randomColor()"
    ...
    <div class="flex justify-start gap-x-3">
      ...
      <img alt="edit"
        class=" cursor-pointer bg-emerald-600 rounded-full w-[30px] md:w-[35px] h-[30px] md:h-[35px] p-2 shadow-2xl self-center hover:scale-[105%] duration-300"
        src="@/assets/icons/edit_note.svg" @click="editPostIt(postIt._id)" />


      <img alt="delete"
        class="cursor-pointer bg-pink-700 rounded-full w-[27px] md:w-[30px] h-[27px] md:h-[30px] p-[7px] shadow-2xl self-center hover:scale-[105%] duration-300"
        src="@/assets/icons/delete.svg" @click="deletePostIt(postIt._id)" />


      <RouterLink
        class=" cursor-pointer px-3 bg-gray-200 py-0 w-auto rounded-lg text-sm flex items-center gap-x-2 text-xs md:text-sm hover:scale-[105%] duration-300"
        :to="{ name: 'details', params: { id: postIt._id } }">view more
